...
while (scanner.hasNext()) {
	...
	var i = 0
	numbers.add(parseRawNumber(rawNumber, { i }, increaseIndex = { i++ }))
}
...
for (i in 0 until numbers.size) {
	for (j in 0 until numbers.size) {
		val firstOperand = numbers[i].copy()
		val secondOperand = numbers[j].copy()
		val magnitude = (firstOperand + secondOperand).getMagnitude()
...
fun SnailfishNumber.copy(): SnailfishNumber =
	when(this) {
		is SnailfishNumber.RegularNumber -> {
			SnailfishNumber.RegularNumber(value = value, parent = null)
		}
		is SnailfishNumber.Pair -> {
			val leftCopy = left.copy()
			leftCopy.isLeft = true
			val rightCopy = right.copy()
			rightCopy.isLeft = false

			val copy = SnailfishNumber.Pair(
				left = leftCopy,
				right = rightCopy,
				parent = null
			)
...
println("Largest magnitude of any sum is $highestMagnitude")
		...
		when (token) {
			...
			']' -> {
				...
				snailfishNumber = SnailfishNumber.RegularNumber(
					parent = null,
					value = Character.getNumericValue(token).takeIf { it >= 0 } ?: throw IllegalStateException("The value parsed should be a number")
				)
				increaseIndex()
			}
		}
